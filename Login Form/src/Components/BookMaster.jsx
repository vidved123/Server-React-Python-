...
const fetchBookMasterData = async () => {
    ...
    if (!token) return []; // Return an empty array to avoid breaking the UI

    try {
        const response = await axios.get("http://127.0.0.1:5000/book_master", {
            headers: {
        ...
        return response.data.books || []; // Ensure books is an array
    } catch (error) {
