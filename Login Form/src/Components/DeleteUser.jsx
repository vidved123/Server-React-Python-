...
export default function DeleteUser() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:8080/delete_user", {
  headers: {
  ...
  withCredentials: true,
    ...
    const handleDelete = async (e) => {
        ...
        try {
            await axios.delete("http://127.0.0.1:8080/delete_user", {
  headers: {
    Authorization: `Bearer ${localStorage.getItem("token")}`,
  },
  withCredentials: true,
    ...
    return (
        <div className="delete-user-container">
            ...
            <div className="back-button">
                <button onClick={() => navigate("/dashboard")}>Back To The Dashboard</button>
            </div>
        </div>
    );
}
